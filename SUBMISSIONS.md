...
weights or setting up a larger submission system for a one-hour club competition.

## Google Form settings

- Collect email addresses.
- Limit to one response, but allow members to edit until the deadline.
...
3. Paste both member cells into their matching form fields.
...
1. Treat member-entered public scores as provisional.
...
6. Keep the private evaluator and organizer benchmark inaccessible until the competition
   and any appeals are complete.

Add five organizer-only columns to the linked response sheet: `Eligible`, `Hidden MAE`,
`Verified parameters`, `Inference ms`, and `Review note`. This keeps the original form
response untouched while giving you one auditable leaderboard. Publish only member
name, rounded hidden MAE, parameters, and rank unless members consent to sharing more.
